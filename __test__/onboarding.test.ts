...
describe("onboarding input focus behavior", () => {
  it("does not auto-focus profile fields on screen load", () => {
    const sourcePath = path.join(process.cwd(), "app/(auth)/onboarding/collect-profile.tsx");
...
describe("checkOnboardingStatus", () => {
  ...
  it("should throw ProfileNotFoundError for PGRST116", async () => {
    ...
    await expect(checkOnboardingStatus("missing")).rejects.toBeInstanceOf(ProfileNotFoundError);
  });
